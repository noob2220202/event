...
커스텀 이모지, 굵게, 색상 등 자유롭게 쓰고,

- **당첨자를 태그할 메시지에만** 그 자리에 `@태그` 문자열을 그대로 입력해두세요. (다른
  placeholder를 쓰고 싶으면 `/placeholder` 명령으로 바꿀 수 있습니다.)
- **현재 추첨 대상 인원 수를 보여줄 메시지에는** (예: "추첨 중..." 단계) 그 자리에 `{인원수}`
  문자열을 그대로 입력해두세요. (변경하려면 `/countplaceholder` 명령)
...
대상 채팅의 username/id를 모르거나 그냥 편하게 설정하고 싶으면, **그 그룹/채널 안에서 직접**
다음 명령을 보내면 그 채팅 자체가 대상으로 등록됩니다 (저장된 메시지가 아니라 실제 대상
채팅에 보내야 합니다):
...
`/chat`과 달리 인자가 필요 없고, 등록 후 명령/확인 메시지는 3초 후 자동 삭제되어 채팅방에
흔적이 남지 않습니다. (저장된 메시지에서 보내면 무시됩니다 — 자기 자신은 대상이 될 수 없음.)

### 4) 당첨자 발표 / 참여자 수는 자동

scheduler는 각 단계 발송 직전 그 단계 템플릿 안에:

- 태그 placeholder(`@태그`)가 들어있으면, `/chat`으로 설정한 대상 채팅의 참여자 중 **봇을
  제외하고 무작위로 한 명**을 뽑아 그 자리에 태그해서 보냅니다.
- 참여자 수 placeholder(`{인원수}`)가 들어있으면, 그 시점 대상 채팅의 **봇을 제외한 참여자
  수**를 숫자로 채워서 보냅니다.

두 placeholder는 서로 독립적이라 한 템플릿에 둘 다 넣어도 되고, 단계별로 필요한 것만 넣어도
됩니다. 둘 다 없는 일반 공지 단계는 그대로 발송됩니다.
