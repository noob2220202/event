...
### ⚠️ 프리미엄 커스텀 이모지와 발송 방식 (중요)

**커스텀(프리미엄) 이모지는 "새로 작성한 메시지"로는 텔레그램 프리미엄 계정만 보낼 수 있습니다.**
비프리미엄 계정이 커스텀 이모지가 들어간 메시지를 새로 작성해 보내면, 텔레그램 서버가 커스텀
이모지를 떼어내고 기본 이모지(🙂 등)로 깨뜨립니다. 이건 봇이든 유저 세션이든 동일한 텔레그램의
제약입니다. ("포워딩"은 누구나 이모지를 보존합니다 — 그래서 저장된 메시지에서는 멀쩡히 보이는
것입니다.)

그래서 scheduler는 로그인한 계정의 프리미엄 여부를 자동으로 감지해 발송 방식을 바꿉니다:

- **프리미엄 계정** → 원본 text+entities를 그대로 재합성해서 발송합니다. 커스텀 이모지가 보존되고
  `@태그`/`{인원수}`도 메시지 안에서 바로 치환됩니다. (원하는 결과 그대로, 권장)
- **비프리미엄 계정** → 원본 템플릿을 그대로 **포워딩**(`drop_author`, '전달됨' 표시 없음)해서
  커스텀 이모지를 보존합니다. 당첨자/참여자 수가 필요한 단계는, 포워딩된 메시지에 **답장**으로
  `🎉 당첨자: @아무개` / `👥 참가자: N명` 줄을 따로 붙여 보냅니다.
  - 이때 템플릿 안의 `@태그`/`{인원수}` 글자는 포워딩이라 그대로(치환 안 된 채) 보입니다.
    깔끔하게 하려면 **비프리미엄 모드에서는 당첨자 발표 템플릿에서 `@태그` 줄을 빼고**, 봇이
    답장으로 붙이는 당첨자 줄에 맡기세요.

> 결론: 사진처럼 **꾸민 메시지 안에 당첨자 태그가 인라인으로** 들어가길 원하면, **발송 계정에
> 텔레그램 프리미엄을 켜는 것**이 유일하게 완벽한 방법입니다. 프리미엄이 켜져 있으면 위의 모든
> 단계가 추가 설정 없이 사진과 동일하게 동작합니다.
